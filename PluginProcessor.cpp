...
void MakoDist01AudioProcessor::Mako_Program_Init()
{
    ...
    Mod1_B_Ring1_Max[0] = 6000;                  //R4.00 Size of buffer.
    Mako_FX_Setup_Mod1();

    //R4.10 Pedal 1 DC Removal. Bronzer adds a DC offset to the signal that needs to bre removed.
    Filter_HP_Coeffs(20.0f, &fil_Mod1_DCRemoval);
    ...
    Mod3_B_Ring1_Max[0] = 6000;                //R4.10 Size of buffer.    
    Mod3_B_Ring1_Max[1] = 6000;                //R4.10 Size of buffer.
    Mako_FX_Setup_Mod3();

    //R4.00 MODULATE 4 Vars - WIDE LOAD.
    Mod4_B_Ring1[0] = 0;                       //R4.00 index into buffer.
    Mod4_B_Ring1[1] = 0;
    Mod4_B_Ring1_Max[0] = 6000;                //R4.00 Size of buffer.
    Mod4_B_Ring1_Max[1] = 6000;
...
{   
    ...
    //R4.20 Calc Up Sample filter.
    Filter_LP_Coeffs_4xUp(10000.0f, &fil_AmpUpSamp);
...
{        
    ...
    Mod1_B_Offset_Max[0] = 20 + int(500.0f * Setting[e_Mod1Depth]);
}
...
void MakoDist01AudioProcessor::Mako_FX_Setup_Mod3()
{
    ...
    //R4.10 Verify our Min,Max limits are within the buffer size.
    //R4.10 Should not be needed since our buffer is 8000 samples and the largest
    //R4.10 possible Min/Max (30mS) is at 192 kHz which = 5760 samples.
    for (int t = 0; t < 2; t++)
    {
...
void MakoDist01AudioProcessor::Mako_FX_Setup_Mod4()
{
    ...
    //R4.10 
    Mod4_Depth[0] = abs(Setting[e_Mod4Doff]) * (Mod4_B_Ring1_Max[0] - 1);
...
/*
// PARTIAL UP SAMPLING - Completed in function below _4x
//R4.00 The heart of MD2. This func applies an IR to incoming samples that represents
//R4.00 the amplifier preamp EQ. It also applies gain, thinning, etc.
float MakoDist01AudioProcessor::Mako_Amp_InputIR(float tSample, int channel)
{
    ...
    if (0.0f < Setting[e_AmpGain])
    {
        ...
        for (int SampX = 0; SampX < 4; SampX++)
        {
            ...
            //R4.20 4x Up Sampling filter.
            V = Filter_Calc_BiQuad(V, channel, &fil_AmpUpSamp);
        }
    }
...
float MakoDist01AudioProcessor::Mako_Amp_InputIR(float tSample, int channel)
{
    ...
    float BW = abs(V - AmpSample_Last);
    if (.25f < BW)
    {
        if (AmpSample_Last < V)
            V = AmpSample_Last + .25f;
        else
            V = AmpSample_Last - .25f;
    }
    AmpSample_Last = V;
...
void MakoDist01AudioProcessor::Filter_LP_Coeffs_4xUp(float fc, tp_filter* fn)
{
...
void MakoDist01AudioProcessor::Mako_FX_Setup_Distortion1(int pos)
{
    ...
    Filter_BP_Coeffs((-.5f + Setting[e_Dist1Mid]) * 30.0f, Setting[e_Dist1MidF], .1f + (Setting[e_Dist1MidQ] * 2.9f), &makoPedal[pos].Filter1);
...
{
    //R4.20 Changed Mid filter gain.
    Filter_BP_Coeffs((-.5f + Setting[e_Dist2Mid]) * 30.0f, Setting[e_Dist2MidF], .1f + (Setting[e_Dist2MidQ] * 2.9f), &makoPedal[pos].Filter1);
...
float MakoDist01AudioProcessor::Mako_FX_Audio_Flange(float tSample, int channel)
{
    ...
    //R4.10 Alter our delay direction, stay between MIN and MAX.
    if (Mod1_B_Offset[channel] < Mod1_B_Offset_Min[channel]) {
        Mod1_B_Offset[channel] = Mod1_B_Offset_Min[channel];
        Mod1_B_Rate[channel] = Mod1_B_Rate_Up; 
    }
    if (Mod1_B_Offset_Max[channel] < Mod1_B_Offset[channel])
